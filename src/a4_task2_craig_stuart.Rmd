...
**Figure 4**: A linear regression model of the relationship between juvenile snowshoe hare hind foot length (mm) and weight (g) from Bonanza Creek LTER Experimental Forest study. Linear model summary: $\beta$~1~ = 9.52 g mm^-1^, p < 0.001, R^2^ = 0.30, Pearson's *r* = 0.55) Data: Kielland et. al. (2017).

- Figure 3 shows the general relationship between hind food length and weight of juvenile snowshoe hares. Figure 4 shows that this relationship appears to have a slight linear relationship (p < 0.001, R^2^ = 0.30) with an average slope of $\beta$ = 9.52 g mm^-1^ (i.e., for each one millimeter increase in hind foot length we expect an average increase in hare weight of 9.52 g). The relationship between hind foot length and weight of juvenile Snowshoe hare is moderately, and significantly, positively correlated (Pearson's *r* = 0.55, p < 0.001). However, diagnostic plots (not included in report) reveal that the assumption of homoscedasticity is violated because the data seems to have a heteroscedastic spread of residuals about the model. Even though the linear regression statistics point to a significant and positive linear relationship, the assumptions tells us otherwise, and without checking the assumptions, you could incorrectly interpret your data for something that it's not.  
...
- Snowshoe hare hind foot length and weight appear to have a significant linear relationship with a slope (expected average increase in weight with a 1 mm incense in hind foot length) of 9.52 g mm^-1^.
- linear regression diagnostic plots reveal that the homoscedasticity assumption is violated. 

Next steps:

- Compare hind foot length and weight relationship between grids. 
